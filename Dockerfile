FROM pytorch/pytorch:2.8.0-cuda12.8-cudnn9-runtime

ENV DEBIAN_FRONTEND=noninteractive \
    PYTHONUNBUFFERED=1 \
    PIP_NO_CACHE_DIR=1 \
    HF_HOME=/root/.cache/huggingface \
    WHISPER_MODEL_ID=Systran/faster-whisper-large-v3 \
    WHISPER_COMPUTE_TYPE=float16 \
    WHISPER_BATCH_SIZE=16 \
    MULTILINGUAL_CHUNK_LENGTH_SEC=8 \
    MAX_AUDIO_BYTES=536870912 \
    ALLOWED_AUDIO_HOST_SUFFIX=.r2.cloudflarestorage.com

RUN apt-get update \
    && apt-get install -y --no-install-recommends ffmpeg ca-certificates \
    && rm -rf /var/lib/apt/lists/*

WORKDIR /app

COPY requirements.txt .
RUN python -m pip install --no-cache-dir -r requirements.txt

COPY handler.py language_selection.py log_safety.py output_payload.py result_sidecar.py speaker_segments.py ./

# Keep the explicit image manifest honest. These lightweight local-module
# imports fail the image build if a handler dependency is ever omitted again,
# instead of letting RunPod accept jobs into a worker that crashes at startup.
RUN python -c "import language_selection, log_safety, output_payload, result_sidecar, speaker_segments"

CMD ["python", "-u", "handler.py"]
